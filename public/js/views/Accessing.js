...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    obj.events=function(parameters){
        ...
        parameters.loginForm.find(".buttonBlack").click(function(){
    ...
    /**
     *  Main method to signup with Facebook aplication, executed when the signup event is executed
     *  @param {Object} form DatosFacebook
     **/
    obj.signupFacebook=function(form){
        var fields=obj.signupValidate(form);
        if(fields){
            maqinato.ajax.signup(fields.email,fields.password,fields.name,fields.lastname,function(response){
                if(response==="exist"){
